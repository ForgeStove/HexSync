...
public class SettingsJDialog {
	...
	public static void initSettingsJDialog() {
		...
		loadConfig();
		// 设置对话框
		var settingsJDialog = new JDialog(frame, "设置");
		...
		var serverPortField = new JTextField(String.valueOf(serverPort));
		serverPanel.add(serverPortField);
		serverPanel.add(new JLabel("<html>最大上传速率:"));
		var serverUploadRateLimitField = new JTextField(String.valueOf(serverUploadRateLimit));
		...
		var serverSyncDirectoryField = new JTextField(serverSyncDirectory);
		serverPanel.add(serverSyncDirectoryField);
		var serverAutoStartBox = newJCheckBox(serverPanel, "<html>自动启动服务端", serverAutoStart);
		tabbedPane.addTab("<html>服务端设置", serverPanel);
		// 客户端选项卡
		var clientPanel = new JPanel(new GridLayout(5, 2));
		clientPanel.add(new JLabel("<html>端口号:"));
		var clientPortField = new JTextField(String.valueOf(clientPort));
		clientPanel.add(clientPortField);
		clientPanel.add(new JLabel("<html>服务器地址:"));
		var serverAddressField = new JTextField(serverAddress);
		clientPanel.add(serverAddressField);
		clientPanel.add(new JLabel("<html>客户端同步路径:"));
		var clientSyncDirectoryField = new JTextField(clientSyncDirectory);
		clientPanel.add(clientSyncDirectoryField);
		clientPanel.add(new JLabel("<html>仅客户端模组路径:"));
		var clientOnlyDirectoryField = new JTextField(clientOnlyDirectory);
		clientPanel.add(clientOnlyDirectoryField);
		var clientAutoStartBox = newJCheckBox(clientPanel, "<html>自动启动客户端", clientAutoStart);
