...
public class HexSync extends JFrame {
	...
	private static void saveConfig() {
		// 保存服务端的配置
		File serverConfigFile = new File(CONFIG_FILE_PATH);
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(serverConfigFile))) {
			for (String line : configContent) writer.write(line + "\n"); // 写入配置内容
			LOGGER.log(Level.INFO, "写入客户端配置: \n" + Arrays.toString(configContent));
		} catch (IOException error) {
			LOGGER.log(Level.SEVERE, "保存客户端配置文件时出错: " + error.getMessage());
		}
	}
	// 发送响应
	...
	private void openSettingsDialog() {
		...
		// 保存按钮
		JButton serverSaveButton = new JButton("保存服务端设置");
		// 修改保存按钮的逻辑
		serverSaveButton.addActionListener(event -> {
			// 检查输入框是否为空
			if (isTextFieldEmpty(serverPortField, "服务端端口") || isTextFieldEmpty(serverSyncDirectoryNameField, "服务端同步文件夹名称") || isTextFieldEmpty(serverSyncDirectoryPathField, "服务端同步文件夹路径"))
				return;
			// 检测端口号是否是数字
			if (isNotNumber(serverPortField.getText().trim()) || Integer.parseInt(serverPortField.getText().trim()) < 1 || Integer.parseInt(serverPortField.getText().trim()) > 65535)
				return;
			...
			else if (uploadRateLimitText.isEmpty()) uploadRateLimit = 0;
			else uploadRateLimit = Integer.parseInt(uploadRateLimitText);
		...
		// 客户端保存按钮
		JButton clientSaveButton = new JButton("保存客户端设置");
		clientSaveButton.addActionListener(event -> {
			// 检查输入框是否为空
			if (isTextFieldEmpty(clientPortField, "客户端端口") || isTextFieldEmpty(clientSyncDirectoryNameField, "客户端同步文件夹名称") || isTextFieldEmpty(clientSyncDirectoryPathField, "客户端同步文件夹路径"))
				return;
			else if (isNotNumber(clientPortField.getText().trim()) || Integer.parseInt(clientPortField.getText().trim()) < 1 || Integer.parseInt(clientPortField.getText().trim()) > 65535) {
				LOGGER.log(Level.WARNING, "客户端端口号错误");
				return;
			}
		...
		// 按钮面板
		JPanel buttonPanel = new JPanel();
		buttonPanel.add(serverSaveButton);
		buttonPanel.add(clientSaveButton);
